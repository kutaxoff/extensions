  ...
  _ampmClicked(source: MtxAMPM): void {
    ...
    const newActiveDate = this._adapter.clampDate(
      this._adapter.createDatetime(
        ...
        this._adapter.getSecond(this._activeDate)
      ),
      this.minDate,
      this.maxDate
    );
    ...
    switch (event.keyCode) {
      case UP_ARROW:
        if (this._clockView === 'hour') {
          ...
          this._activeDate = this._adapter.addCalendarMinutes(this._activeDate, this.timeInterval);
        } else if (this._clockView === 'second') {
          this._activeDate = this._adapter.addCalendarSeconds(this._activeDate, this.timeInterval);
        }
        break;
      case DOWN_ARROW:
        if (this._clockView === 'hour') {
